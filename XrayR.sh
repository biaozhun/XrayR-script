...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontents.com/biaozhun/XrayR-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontents.com/biaozhun/XrayR-script/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
