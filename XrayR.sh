...
install() {
    bash <(curl -Ls https://raw.githubusercontents.com/biaozhun/XrayR-script/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontents.com/biaozhun/XrayR-script/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontents.com/biaozhun/XrayR-script/main/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
